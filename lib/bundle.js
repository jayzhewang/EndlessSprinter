	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function draw(ctx) {
	      ...
	      this.score += 1;
	      this.endScore = Math.floor(this.score / 30);
	      if (this.endScore >= 1000) {
	        this.scoreMargin = 84;
	      } else if (this.endScore >= 100) {
	        this.scoreMargin = 56;
	      } else if (this.endScore >= 10) {
	        this.scoreMargin = 28;
	      }
	      ...
	      ctx.font = "50px Arial";
	      ctx.strokeText('score: ' + this.endScore, 600 - this.scoreMargin, 50);
	  ...
	  _createClass(Land, [{
	    ...
	    value: function draw(ctx) {
	      ...
	      this.currentLand.forEach(function (piece) {
	        ctx.drawImage(image, 0, 0, 80, 100, piece.x, piece.y, piece.width - 16, piece.height);
	      });
	    }
	  }]);
...
/***/ function(module, exports) {
	...
	var GameView = function () {
	  ...
	  _createClass(GameView, [{
	    ...
	    value: function displayHighScores(highScores) {
	      var _this2 = this;
	      ...
	      var scoreSection = $('.highscores-table');
	
	      var _loop = function _loop(i) {
	        var name = scores[i]['name'];
	        scoreSection.append('\n        <tr id=\'player' + i + '\'>\n          <td>' + (i + 1) + '</td>\n          <td>' + name + '</td>\n          <td>' + _this2.scorePadding(scores[i]['score']) + '</td>\n        </tr>\n      ');
	        if (name === 'player') {
	          $('#player' + i).addClass('default-player');
	        } else if (name === _this2.userInput) {
	    ...
	    key: 'scorePadding',
	    value: function scorePadding(score) {
	      var padded = '0000' + score;
	      return padded.slice(score.toString().length - 1, padded.length);
	    }
	  }, {
	    key: 'reDisplayHighScores',
	    value: function reDisplayHighScores(score) {
	      this.displayForm = false;
	      $("form").remove();
	      $('.info-inner').empty();
	    ...
	    value: function fetchScores() {
	      ...
	      $.ajax({
	        ...
	        crossDomain: true,
	        success: function success(scores) {
	          _this3.highScores = scores;
	        }
	      });
	    }
	  }, {
	    key: 'createScore',
	    value: function createScore(player, score) {
	      var _this4 = this;
	      ...
	      $.ajax({
	        ...
	        url: 'https://esrails.herokuapp.com/scores',
	        crossDomain: true,
	        success: function success(singleScore) {
	          _this4.reDisplayHighScores(singleScore);
	        }
	      });
	    }
	  }, {
	    key: 'showScoreForm',
	    value: function showScoreForm() {
	      var _this5 = this;
	
	      var scoreSection = $('.highscores');
	      scoreSection.append('\n      <form>\n        <div class=\'submission\'>\n          <input id=\'nameinput\' type=\'text\' placeholder=\'Enter your name\'>\n          <input id=\'submit\' type=\'submit\' value=\'submit\'>\n        </div>\n      </form>');
	      $("form").submit(function (e) {
	        e.preventDefault();
	        _this5.userInput = $("#nameinput").val();
	        if (_this5.userInput.length > 2 && _this5.userInput.length < 9) {
	          $("form").remove();
	          _this5.createScore(_this5.userInput, _this5.game.endScore);
	        } else {
	    ...
	    key: 'compareScores',
	    value: function compareScores() {
	      for (var i = 0; i < this.highScores.length; i++) {
	        if (this.highScores.length === 0 || this.highScores[i]['score'] < this.game.endScore) {
	    ...
	    value: function explode() {
	      this.ctx.clearRect(this.sprinterCoords[0] - 40, this.sprinterCoords[1] - 40, this.sprinterCoords[2] + 40, this.sprinterCoords[3] + 40);
	      this.ctx.fillStyle = '#fafafa';
	      this.ctx.fillRect(this.sprinterCoords[0] - 40, this.sprinterCoords[1] - 40, this.sprinterCoords[2] + 40, this.sprinterCoords[3] + 40);
	      this.ctx.drawImage(explosion, explodeCoords[this.idx][0], explodeCoords[this.idx][1], 95, 95, this.sprinterCoords[0] - 40, this.sprinterCoords[1] - 40, this.sprinterCoords[2] + 40, this.sprinterCoords[3] + 40);
	      if (this.idx === 15) {
